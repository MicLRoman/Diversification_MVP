...
def register_admin_handlers(bot: TeleBot, db, app_id, admin_ids):
    ...
    def get_full_report(message):
        ...
        try:
            ...
            auto_clicks = len(auto_selection_users)
            slider_uses = len([d for d in docs if d.to_dict().get('eventName') == 'risk_slider_used'])
            asset_swaps = len([d for d in docs if d.to_dict().get('eventName') == 'asset_replaced'])

            h3_report = (
                f"Гипотеза 3: Обучение на практике\n"
                f"  - Выбор пути: Автоподбор ({auto_clicks}) vs Демо ({demo_clicks})\n"
                f"  - В редакторе: Слайдер риска ({slider_uses}) vs Замена активов ({asset_swaps})\n"
            )
            ...
            # --- АНАЛИЗ АУДИТОРИИ КОНВЕРСИЙ (ИЗМЕНЕНО) ---
            if final_conversion_events:
                # Фильтруем только события, где есть данные опроса
                ...
                if surveyed_events:
                    ...
                    # Собираем и считаем все виды деятельности
                    all_activities = []
                    for e in surveyed_events:
                        activities = e['eventData'].get('user_activities')
                        if activities:
                    ...
                    def format_counter(counter_list):
                        if not counter_list: return "Нет данных"
                        return ", ".join([f"'{k}': {v}" for k, v in counter_list])
